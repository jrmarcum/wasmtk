...
export function bits(x: number): bigint {
  dv.setFloat64(0, x);
...
export function toBig(x: number): bigint {
  ...
  const sign = (b >> 63n) ? -1n : 1n;
  const exp = Number((b >> 52n) & 0x7ffn);
  let mant = b & 0xfffffffffffffn;
  let e2: number, m: bigint;
  if (exp === 0) {
    m = mant;
    e2 = -1074;
  } else {
...
export function roundScaled(a: bigint, e: number): number {
  ...
  if (keep < 1) { // below the smallest subnormal
    const cmpExp = -1075 - e;
    if (cmpExp < 0) return neg ? -Number.MIN_VALUE : Number.MIN_VALUE;
    const thr = 1n << BigInt(cmpExp);
    if (a > thr) return neg ? -Number.MIN_VALUE : Number.MIN_VALUE;
    return neg ? -0 : 0;
  }
  const drop = bl - keep;
  let mant: bigint, resExp: number;
  if (drop > 0) {
    const half = 1n << BigInt(drop - 1);
    const low = a & ((1n << BigInt(drop)) - 1n);
    mant = a >> BigInt(drop);
    if (low > half || (low === half && (mant & 1n) === 1n)) mant += 1n;
    resExp = e + drop;
  } else {
    mant = a << BigInt(-drop);
    resExp = e + drop;
  }
  const val = Number(mant) * Math.pow(2, resExp);
  return neg ? -val : val;
}
...
export function decomp(x: number): { m: number; e: number } {
  dv.setFloat64(0, x);
  let b = dv.getBigUint64(0);
  let exp = Number((b >> 52n) & 0x7ffn);
  let e: number;
  if (exp === 0) {
    x *= 2 ** 54;
    dv.setFloat64(0, x);
    b = dv.getBigUint64(0);
    exp = Number((b >> 52n) & 0x7ffn);
    e = exp - 1023 - 54;
  } else e = exp - 1023;
  const mb = (b & 0x800fffffffffffffn) | 0x3ff0000000000000n;
  dv.setBigUint64(0, mb);
...
export function twoProd(a: number, b: number): DD {
  const p = a * b;
  let c = SPLIT * a;
  const ah = c - (c - a), al = a - ah;
  c = SPLIT * b;
  const bh = c - (c - b), bl = b - bh;
  return [p, ((ah * bh - p) + ah * bl + al * bh) + al * bl];
}
export function twoSum(a: number, b: number): DD {
  const s = a + b, bb = s - a, e = (a - (s - bb)) + (b - bb);
  return [s, e];
}
...
export function ddRecipInt(d: number): DD {
  const h = 1 / d;
  const [p, e] = twoProd(h, d);
  const res = (1 - p) - e;
  return twoSum(h, res * h);
}
...
export function crRound(hi: number, lo: number, k: number): number {
  ...
  const eh = Math.floor(Math.log2(hi));
  if (eh + k > 1023) return neg ? -Infinity : Infinity;
  const probe = scalbn(hi, k);
  let val: number;
  if (probe >= 2.2250738585072014e-308) val = scalbn(hi + lo, k);
  // normal: round-then-scale
  else { // subnormal: round (hi+lo)*2^(k+1074) to nearest even integer, then *2^-1074
    const j = k + 1074;
    let whi = scalbn(hi, j), wlo = scalbn(lo, j);
    let fl = Math.floor(whi);
    let frac = (whi - fl) + wlo;
    while (frac >= 1) {
      frac -= 1;
      fl += 1;
    ...
    let m: number;
    if (frac > 0.5) m = fl + 1;
    else if (frac < 0.5) m = fl;
    else m = (fl % 2 === 0) ? fl : fl + 1;
...
export function expOracle(x: number): number {
  ...
  const xs = toBig(x);
  let k = (xs * 2n + (xs >= 0n ? LN2 : -LN2)) / (2n * LN2);
  let r = xs - k * LN2;
...
function logScaled(x: number): bigint {
  let { m, e } = decompBig(x);
  if (m >= SQRT2big) {
    m >>= 1n;
    e += 1;
  }
  const s = ((m - SCALE) * SCALE) / (m + SCALE);
  return BigInt(e) * LN2 + atanh2(s);
}
function decompBig(x: number): { m: bigint; e: number } {
  dv.setFloat64(0, x);
  const b = dv.getBigUint64(0);
  const exp = Number((b >> 52n) & 0x7ffn);
  let mant = b & 0xfffffffffffffn;
  let e2: number, mm: bigint;
  if (exp === 0) {
    mm = mant;
    e2 = -1074;
  } else {
  ...
  const bl = mm.toString(2).length;
  const e = e2 + bl - 1;
  const shift = Number(P) - (bl - 1);
  const m = shift >= 0 ? mm << BigInt(shift) : mm >> BigInt(-shift);
  return { m, e };
}
...
function atanh2(s: bigint): bigint {
  ...
  for (let k = 1n; k < 200n; k++) {
    term = (term * s2) >> P;
    const add = term / (2n * k + 1n);
    sum += add;
    if (add < 2n && add > -2n) break;
  }
  return sum * 2n;
}
function roundBig(total: bigint): number {
  const neg = total < 0n;
  let a = neg ? -total : total;
  if (a === 0n) return 0;
  const bl = a.toString(2).length;
  const e2 = bl - 1 - Number(P);
  const drop = bl - 53;
  let mm: bigint, re: number;
  if (drop > 0) {
    const h = 1n << BigInt(drop - 1);
    const lo = a & ((1n << BigInt(drop)) - 1n);
    mm = a >> BigInt(drop);
    if (lo > h || (lo === h && (mm & 1n) === 1n)) mm += 1n;
    re = e2 - 52;
    if (mm >= (1n << 53n)) {
      mm >>= 1n;
      re += 1;
  ...
  const v = Number(mm) * Math.pow(2, re);
  return neg ? -v : v;
}
export const logOracle = (x: number) =>
  x === 1 ? 0 : x === 0 ? -Infinity : x < 0 ? NaN : !isFinite(x) ? x : roundBig(logScaled(x));
export const log2Oracle = (x: number) =>
  x === 1
    ? 0
    : x === 0
    ? -Infinity
    ...
    : !isFinite(x)
    ? x
    : roundBig((logScaled(x) * SCALE) / LN2);
export const log10Oracle = (x: number) =>
  x === 1
    ? 0
    : x === 0
    ? -Infinity
    ...
    : !isFinite(x)
    ? x
    : roundBig((logScaled(x) * SCALE) / LN10);
export function cbrtOracle(x: number): number {
  if (x !== x) return NaN;
  if (x === 0) return x;
  ...
  const W = toBig(w);
  const W3 = W << (2n * P);
  let Y = toBig(Math.cbrt(w));
  for (let it = 0; it < 200; it++) {
    const nY = (2n * Y + W3 / (Y * Y)) / 3n;
    const d = nY > Y ? nY - Y : Y - nY;
    Y = nY;
    if (d <= 1n) break;
  }
  const r = roundScaled(Y, q - Number(P));
  return neg ? -r : r;
}
// sin/cos/tan oracle: reduce mod 2*pi in fixed-point, Taylor.
function trigReduce(x: number): bigint {
  let r = ((toBig(x) % TWO_PI) + TWO_PI) % TWO_PI;
  if (r > PI) r -= TWO_PI;
  return r;
}
function scSin(r: bigint): bigint {
  const r2 = (r * r) >> P;
  let t = r, s = r;
  for (let k = 1n; k < 90n; k++) {
    t = (t * r2) >> P;
    t = -t / ((2n * k) * (2n * k + 1n));
    s += t;
    if (t > -2n && t < 2n) break;
...
function scCos(r: bigint): bigint {
  const r2 = (r * r) >> P;
  let t = SCALE, s = SCALE;
  for (let k = 1n; k < 90n; k++) {
    t = (t * r2) >> P;
...
export const sinOracle = (x: number) =>
  !isFinite(x) ? NaN : roundScaled(scSin(trigReduce(x)), -Number(P));
export const cosOracle = (x: number) =>
  !isFinite(x) ? NaN : roundScaled(scCos(trigReduce(x)), -Number(P));
export const tanOracle = (x: number) =>
  !isFinite(x)
    ? NaN
    : roundScaled((scSin(trigReduce(x)) * SCALE) / scCos(trigReduce(x)), -Number(P));
...
function atanSeries(A: bigint): bigint {
  ...
  for (let k = 1n; k < 400n; k++) {
    term = (term * A2) >> P;
    const add = (k % 2n === 1n ? -term : term) / (2n * k + 1n);
    sum += add;
    if (add < 2n && add > -2n) break;
  }
  return sum;
...
export function atanOracle(x: number): number {
  ...
  if (A > TANPI8) {
    A = ((A - SCALE) * SCALE) / (A + SCALE);
    mid = true;
  }
  let R = atanSeries(A);
  if (comp && mid) R = PI4 - R;
  else if (comp) R = PI2 - R;
  else if (mid) R = PI4 + R;
  const val = roundScaled(R, -Number(P));
  return neg ? -val : val;
}
...
export function asinOracle(x: number): number {
  ...
  const val = roundScaled(atanFxSigned(t), -Number(P));
  return neg ? -val : val;
}
export function acosOracle(x: number): number {
  ...
  if (Math.abs(x) > 1) return NaN;
  if (x === 1) return 0;
  if (x === -1) return roundScaled(PI, -Number(P));
  const X = toBig(x);
...
export function atan2Oracle(y: number, x: number): number {
  ...
  if (!isFinite(x) || !isFinite(y)) {
    if (!isFinite(x) && !isFinite(y)) {
      const a = x > 0 ? roundScaled(PI4, -Number(P)) : roundScaled(3n * PI4, -Number(P));
      return y > 0 ? a : -a;
    }
    if (!isFinite(y)) return y > 0 ? pi2 : -pi2;
    return x > 0
      ? (y < 0 || Object.is(y, -0) ? -0 : 0)
      : (y < 0 || Object.is(y, -0) ? -piCR : piCR);
  }
...
function expBig(x: number): { m: bigint; e: number } {
  const xs = toBig(x);
  const k = (xs * 2n + (xs >= 0n ? LN2 : -LN2)) / (2n * LN2);
  const r = xs - k * LN2;
  let term = SCALE, sum = SCALE;
...
function alignExp(
  ...
  return [a.m << BigInt(a.e - emin), b.m << BigInt(b.e - emin), emin];
}
export function sinhOracle(x: number): number {
  if (x !== x || !isFinite(x)) return x;
  if (x === 0) return x;
  const [M1, M2, emin] = alignExp(expBig(x), expBig(-x));
  return roundScaled(M1 - M2, emin - 1);
}
...
export function tanhOracle(x: number): number {
  ...
  if (x === -Infinity) return -1;
  const [M1, M2] = alignExp(expBig(x), expBig(-x));
  return roundScaled(((M1 - M2) << P) / (M1 + M2), -Number(P));
}
...
function isqrtBig2(n: bigint): bigint {
  if (n < 2n) return n;
  let x = n, y = (x + 1n) >> 1n;
  while (y < x) {
    x = y;
...
function atanh2s(s: bigint): bigint {
  const s2 = (s * s) >> P;
  let t = s, sum = s;
  for (let k = 1n; k < 300n; k++) {
    t = (t * s2) >> P;
    const a = t / (2n * k + 1n);
    sum += a;
    if (a < 2n && a > -2n) break;
...
function lnScaled(V: bigint): bigint { // ln(V*2^-P)*2^P
  ...
  const sh = Number(P) - (bl - 1);
  let m = sh >= 0 ? (V << BigInt(sh)) : (V >> BigInt(-sh));
  let e = bl - 1 - Number(P);
  if (m >= SQRT2) {
    m >>= 1n;
    e += 1;
...
export function asinhOracle(x: number): number {
  ...
  const A = toBig(a);
  const val = roundScaled(lnScaled(A + sqrtFx2(((A * A) >> P) + SCALE)), -Number(P));
  return neg ? -val : val;
}
export function acoshOracle(x: number): number {
  ...
  if (x === Infinity) return Infinity;
  const A = toBig(x);
  return roundScaled(lnScaled(A + sqrtFx2(((A * A) >> P) - SCALE)), -Number(P));
}
export function atanhOracle(x: number): number {
  ...
  const X = toBig(x);
  return roundScaled(lnScaled(((SCALE + X) << P) / (SCALE - X)), -Number(P) - 1);
}
...
function expFromScaled(A: bigint): number { // correctly-round e^(A*2^-P)
  const k = (A * 2n + (A >= 0n ? LN2 : -LN2)) / (2n * LN2);
  const r = A - k * LN2;
  let term = SCALE, sum = SCALE;
...
export function powOracle(x: number, y: number): number {
  ...
  if (y === 1) return x;
  if (x === 0) {
    if (y < 0) return isOddInt(y) ? (1 / x < 0 ? -Infinity : Infinity) : Infinity;
    return isOddInt(y) ? x : 0;
  }
  const ax = Math.abs(x);
  if (!isFinite(y)) {
    if (ax === 1) return 1;
  ...
  if (!isFinite(x)) {
    const neg = x < 0 && isOddInt(y);
    if (y > 0) return neg ? -Infinity : Infinity;
    return neg ? -0 : 0;
  }
  let sign = 1;
  if (x < 0) {
    if (!Number.isInteger(y)) return NaN;
    if (isOddInt(y)) sign = -1;
  }
