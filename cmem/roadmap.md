...
| **Go producer (TinyGo)** | `tinygo build` → wasm → shared optimize/host path. WASI-first; stdlib `go` heavier fallback. | ✅ **v1 SHIPPED 2026-06-06** — full `tgo-*.ps1` set via `--lang=go` (path defaults to cwd): `init` (scaffold; `--go-target=wasm` for browser), `wasic` (wasip1), `modc` (**browser** `-target=wasm` + `wasm_exec.js`), `run` (build wasip1 + run). `--go-runtime=tinygo`(default)/`std`. `src/gowasic.ts`. **wasm-opt:** real `wasm-opt` → TinyGo full (incl. goroutines); else passthrough shim + `-scheduler=none` + **binaryen-ts `-Oz`** (no external binaryen; goroutine-free). Verified end-to-end. **Deferred:** Go string/aggregate bindgen (needs ABI forward-alignment) + a real reactor/library (`//go:wasmexport`) for the DLL model (the shipped `modc --lang=go` is the *browser* build, per the owner's script naming). Fixture: `tests/go_fixtures/hello.go` (not auto-run — needs TinyGo). |
