...
(SPEC §10 loader-cap propagation COMPLETE across all 10 ports 2026-07-05); **Go bindgen** string/aggregate host marshalling **✅ SHIPPED 2026-07-08** (fully canonical vs wasmtime — existing bindgen marshals TinyGo unchanged; the only fix was SPEC §10 in the bindgen loader), the **asyncify pass in binaryen-ts + goroutine Go wiring** (✅ **COMPLETE end-to-end 2026-07-08** — the pass's in-wasm asyncify-import mode shipped in binaryen-ts 1.4.1, and wasmtk's `--lang=go` now builds goroutine code with `-scheduler=asyncify` + a passthrough shim + `binaryenAsyncify` (binaryen-ts Asyncify+`-Oz`) → goroutines run with NO external binaryen; `tests/go_asyncify_tests.ts` `sum: 30`), and the optional field-reshaping **utility-types** batch (Pick/Omit/Record/…).
