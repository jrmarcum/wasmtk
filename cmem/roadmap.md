...
ABI forward-alignment), the **asyncify pass in binaryen-ts** (🔨 IN PROGRESS — Stage 1/5 done
2026-07-05; unblocks goroutine Go without external `wasm-opt`), and the optional field-reshaping
**utility-types** batch (Pick/Omit/Record/…).
...
| **asyncify pass in binaryen-ts** | Port binaryen's `--asyncify` pass into `@jrmarcum/binaryen-ts` so wasmtk can be TinyGo's `wasm-opt` for **goroutine** code too (no external binaryen at all). Today binaryen-ts has `-Oz` but NOT asyncify, so goroutine Go needs a real `wasm-opt` installed. | 🔨 **IN PROGRESS (Stage 1 of 5 done 2026-07-05).** Faithful port of upstream `Asyncify.cpp` (2030 LOC) into native TS — done IN the binaryen-ts repo (`src/passes/asyncify.ts`; commit `2902fca`), authoritative memory in binaryen-ts `cmem/passes.md` § "Asyncify". **Foundation better than expected:** binaryen-ts already has the 66 IR builders, CFG+liveness (`cfg.ts`/`coalesce-locals.ts`), pass framework; upstream `Asyncify.cpp` + `test_asyncify.py` vendored; **real `wasm-opt` v130 + TinyGo 0.41.1 installed → differential oracle + goroutine e2e**. **Stage 1 ✅** runtime-support synthesis (2 globals + 5 control fns), byte-for-shape validated vs wasm-opt v130, 7 tests, suite 348/348. **Remaining:** Stage 2 ModuleAnalyzer (NEXT) / Stage 3 AsyncifyFlow / Stage 4 AsyncifyLocals / Stage 5 (test_asyncify.py + TinyGo e2e + register + `--asyncify` CLI). Pass unregistered until Stage 4. Unblocks goroutine Go on the binaryen-ts path. |
...
aggregate marshalling, the binaryen-ts asyncify pass (🔨 IN PROGRESS — Stage 1/5 done 2026-07-05;
detail in binaryen-ts `cmem/passes.md` § "Asyncify"), the optional utility-types batch, and the
