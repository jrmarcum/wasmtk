...
(SPEC §10 loader-cap propagation COMPLETE across all 10 ports 2026-07-05); **Go bindgen** string/aggregate host marshalling **✅ SHIPPED 2026-07-08** (fully canonical vs wasmtime — existing bindgen marshals TinyGo unchanged; the only fix was SPEC §10 in the bindgen loader), the **asyncify pass in binaryen-ts** (✅ THE PASS IS COMPLETE 2026-07-05 — 5 stages, registered, runnable e2e matches wasm-opt v130; remaining wasmtk-side = publish + wire into `--lang=go`), and the optional field-reshaping **utility-types** batch (Pick/Omit/Record/…).
...
aggregate marshalling~~ (✅ SHIPPED 2026-07-08 — canonical; SPEC §10 bindgen-loader fix); the binaryen-ts asyncify pass (✅ THE PASS IS COMPLETE 2026-07-05; remaining = wasmtk-side --lang=go wiring + publish; detail in binaryen-ts `cmem/passes.md` § "Asyncify"), the optional utility-types batch, and the
