...
ABI forward-alignment), the **asyncify pass in binaryen-ts** (✅ THE PASS IS COMPLETE 2026-07-05 — 5 stages, registered, runnable e2e matches wasm-opt v130; remaining wasmtk-side = publish + wire into `--lang=go`), and the optional field-reshaping **utility-types** batch (Pick/Omit/Record/…).
...
aggregate marshalling, the binaryen-ts asyncify pass (✅ THE PASS IS COMPLETE 2026-07-05; remaining = wasmtk-side --lang=go wiring + publish; detail in binaryen-ts `cmem/passes.md` § "Asyncify"), the optional utility-types batch, and the
