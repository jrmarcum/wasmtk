...
Live record of bugs found + fixed. Newest first. **✅ NO SUITE-FAILING BUGS — full suite 372/372** (`bindgen` 131/131, `jstyper` 73/73, `dync` conformance 3/3) as of **2026-06-30** (#14 Route A `18zh`–`18zz` + the 5 low-priority-gap regressions `62_Gap*` + 3 discovered-bug regressions `63_Gap*` + 4 user-report regressions `64_Report*`).
...
- **Issue 5 (LOW/MED — NOT fixed):** `Math.sin` diverges ~7th sig-fig in WASM vs the JS engine — a mathlib polynomial-approximation accuracy limit (the trig funcs go through the merged `mathlib.wasm`, not a native intrinsic). Numeric, no clean fix; documented.
