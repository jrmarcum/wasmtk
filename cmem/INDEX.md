...
| File | What it holds |
| --- | --- |
| [best-practices.md](best-practices.md) | **METHOD, not findings — how to work here so the same class of defect does not recur.** Adopted 2026-08-24 from the sibling **wazmrt** project, which pioneered the format; imported rules are marked [wazmrt], ones this project paid for are marked [wasmtk]. **Read before a conformance pass, an audit, a backend bump, or any change to a producer/consumer pair.** The load-bearing one: *a round-trip proves agreement with yourself* — a V8-only oracle held the wasi suite at 417/417 while every `try`/`catch` module was unrunnable on wasmtime. **Added 2026-08-25:** read a hang's CPU time before theorising (spinning ≠ blocked); check the suspect is REACHABLE from the failing input before bisecting it; when a text-parsing fix leaves the symptom, grep for siblings; and stopping a background suite kills the shell, not its children. **2026-08-27 — the STANDING QUESTION shared with binaryang: *where is this construct USED, not where is it NAMED?*** with a **nine-item checklist**, because this pattern is **not detectable by being careful, only by being enumerable** — the newest entries came from two consecutive wrong scope measurements on the same question, the second of which felt more rigorous because it was a correction. Plus **a backlog decays silently — sweep HEADINGS as well as bullets**, after counting open items surfaced stale entries three times in two days. Plus §4b, *tooling that lies to you*: author content with a real file write (a heredoc wrote a NUL byte into `.gitattributes`), never pipe when the exit code IS the measurement, reload before concluding a version does not exist, and prefer a wildcard-plus-exceptions default to a hand-maintained list |
| [workflow.md](workflow.md) | **The working loop: test → bisect → branch-on-fix → gate → memory → commit → merge → bump → publish.** **Step 2b of the release flow writes the DENO FLOOR** — decided 2026-09-20, executed at the next release only, then held. **Also holds the `upstream/binaryang` co-development setup (2026-08-27)** — a gitignored local checkout, `deno.upstream.json` for exploration only, and the guard that `upstream:install` produces `wasmtk-upstream` so an unpublished build can never be picked up by a gate. **A branch is cut only when a test FAILS and needs a `src/` fix** — a clean batch commits straight to `main` (owner directive 2026-07-30). Commit conventions, context boundaries, and the traps that have bitten. **Read at the start of a stress-test batch or a release.** |
...
| [design-decisions.md](design-decisions.md) | Load-bearing invariants that must NOT be silently reverted. **Read before changing codegen or bumping a backend** — holds the **single merged backend `@jrmarcum/binaryang@1.5.3` (wabt-ts + binaryen-ts merged 2026-08-27; two specifiers, `compat/wabt` + `compat/binaryen`, ONE exact version that must move together) and why the caret came off**, the **`minimumDependencyAge` keep-it directive (2026-08-25)**, the invariant that **`grep -rn -F '\(i32\.const' src/*.ts` must come back empty** (never require a bracketing you did not emit), versioning (odometer sequencing, NOT a semver signal), the fmt/CRLF `.gitattributes` rule, and the JSR provenance elimination list |
| [roadmap.md](roadmap.md) | Release status and phase status — **latest is v2.0.2 (2026-08-31): backend on binaryang 1.5.3, +123 wast assertions from `ref.null` marshalling, a Windows write-race fix — PUBLISHED and ATTESTED, and provenance is now TWO-IN-A-ROW after twelve absent releases (cause still unknown, and not ours to claim)**; the README roadmap-table convention. **A BREAKING change goes in its own `### ⚠️ Breaking Changes` table above Feature Status, never as an ordinary row** (owner directive 2026-07-30) |
| [next-work.md](next-work.md) | Short "what to pick up next" list. **⏸️ PAUSED 2026-09-20 — the banner at the top of that file holds the resume sequence and an EXACT acceptance prediction for binaryang's next release (+1/+1, corpus 37,365 → 37,367). Read it first; a different number is a finding, not a detail.** Provenance is RESOLVED (two attested releases), and the `ref_null` and `os error 32` items are DONE.** What remains: `br_on_cast` coordination with binaryang, the 100 pinned wast conformance failures, and the smaller carried items (Deno floor, markdown fmt) |
