...
| [compiler-bugs.md](compiler-bugs.md) | Live bug log — **no open bugs** (suite **287/287**); Phase 51 (2026-06-05) added `instanceof`, closed 3 class gaps (module-level instances, class-instance array literals, single-physical-line class bodies), and a workaround-audit fixed `s.at().charCodeAt()` + added a loud `call_indirect`-in-merge guard (+ a companion **`memory.grow`-in-merge guard 2026-06-07** — rejects foreign growing-heap allocators with a per-language fix message; no false positives, 14/14 merge tests) (+ **reactor `_initialize` fix 2026-06-07** — `wasmtk mod`/`run` now call `_initialize` before a reactor library's export, else it traps `unreachable`; no-op for TS libs); earlier: single-line brace `if {…}` fixed 2026-06-03, 7 long-standing failures fixed 2026-06-02 (value-fallthru + wabt-ts 1.3.1 hex-float), merge OOB-charCodeAt trap fixed 2026-06-02 (short-circuit `&&`/`||`) |
