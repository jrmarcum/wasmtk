...
| [next-work.md](next-work.md) | Short "what to pick up next" list. **Provenance is RESOLVED (two attested releases), and the `ref_null` and `os error 32` items are DONE.** What remains: `br_on_cast` coordination with binaryang, the 100 pinned wast conformance failures, and the smaller carried items (Deno floor, markdown fmt) |
