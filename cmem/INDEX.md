...
| [compiler-bugs.md](compiler-bugs.md) | Live bug log — root cause + fix + regression test for every bug found. **Read before debugging anything.** Also holds the wabt-ts backend findings: 3 execution bugs fixed; **`ref.null` cannot encode for any heap type** (open wabt-ts bug), plus 2 gaps that are parity with upstream wabt — all cost skips, never failures |
