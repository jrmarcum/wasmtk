...
| [polyglot-producers.md](polyglot-producers.md) | **Goal — one congruent polyglot wasm capability** (TS/JS+C+C+++Rust+Zig+Go producers → shared P1-core/bindgen/binaryen-ts/wasmmerge/host back end); WASI-P1 scope pin; **VERIFIED 2026-06-03 wasmtk is NOT a P2 producer**; **DECISION: forward-align the ABI to canonical while staying P1**; ADRs — C/C++ via Zig, Go via TinyGo. **Go producer v1 SHIPPED 2026-06-06** (`src/gowasic.ts`) — full `tgo-*.ps1` set via `--lang=go`: `init`(scaffold; `--go-target=wasm`=browser), `wasic`(wasip1), `modc`(browser `-target=wasm`+`wasm_exec.js`), `run`(build+run); TinyGo/std; wasm-opt auto-fallback = passthrough shim + `-scheduler=none` + binaryen-ts `-Oz` (goroutine-free, no external binaryen); goroutine Go needs real wasm-opt (asyncify). Deferred: Go bindgen + real reactor/library |
