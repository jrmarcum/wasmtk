...
| [workflow.md](workflow.md) | **The working loop: test → bisect → branch-on-fix → gate → memory → commit → merge → bump → publish.** **Also holds the `upstream/binaryang` co-development setup (2026-08-27)** — a gitignored local checkout, `deno.upstream.json` for exploration only, and the guard that `upstream:install` produces `wasmtk-upstream` so an unpublished build can never be picked up by a gate. **A branch is cut only when a test FAILS and needs a `src/` fix** — a clean batch commits straight to `main` (owner directive 2026-07-30). Commit conventions, context boundaries, and the traps that have bitten. **Read at the start of a stress-test batch or a release.** |
