(module
  ...
  (func $mustPositive (param $n f64) (result f64)
    (local $__throw_msg_ptr i32)
    (local $__throw_msg_len i32)
    (local $__tmpl_num_ptr i32)
    (local $__tmpl_num_len i32)
    (if (f64.le (local.get $n) (f64.const 0))
      (then
      (local.set $__throw_msg_ptr (i32.const 260))
      (local.set $__throw_msg_len (i32.const 23))
      (local.set $__tmpl_num_ptr (call $__malloc (i32.const 32)))
      (local.set $__tmpl_num_len (call $__f64_to_str (local.get $n) (local.get $__tmpl_num_ptr)))
  ...
  (func $_start (export "_start")
    (local $__iface_tmp i32)
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__f64_to_str (call $mustPositive (f64.const 5)) (i32.const 132)))
          ...
          (drop (call $fd_write
            ...
            (i32.const 1)
            (i32.const 128)))
    (drop (call $mustPositive (f64.const -1)))
    (call $proc_exit (i32.const 0))
  )
  (data (i32.const 260) "\65\78\70\65\63\74\65\64\20\70\6f\73\69\74\69\76\65\2c\20\67\6f\74\20")
)