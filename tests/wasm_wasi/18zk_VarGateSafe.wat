(module
  ...
  (func $compute  (result f64)
    ...
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (f64.lt (local.get $i) (f64.const 5))))
        (block $cont_0
          (local.set $total (f64.add (local.get $total) (local.get $i)))
        )
        (local.set $i (f64.add (local.get $i) (f64.const 1)))
        (br $loop_0)
      )
    )
    (if (f64.gt (local.get $total) (f64.const 5))
      (then
      (local.set $bonus (f64.const 100))
      (local.set $total (f64.add (local.get $total) (local.get $bonus)))
      )
    )
  ...
  (func $_start (export "_start")
    ...
    (local $__tmpl_num_len i32)
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (i32.store (i32.const 4) (i32.add (i32.const 8) (call $__f64_to_str (call $compute) (i32.const 140))))
