(module
  ...
  (global $message_ptr (mut i32) (i32.const 260))
  ...
  (func $send (param $msg_ptr i32) (param $msg_len i32) 
    (global.set $message_ptr (local.get $msg_ptr))
      (global.set $message_len (local.get $msg_len))
  )

  (func $receive  
    (local $__ret_str_ptr i32)
    (local $__ret_str_len i32)
    (local.set $__ret_str_ptr (global.get $message_ptr))
      (local.set $__ret_str_len (global.get $message_len))
  ...
  (func $_start (export "_start")
    ...
    (call $send (i32.const 260) (i32.const 4))
    (call $receive)
(local.set $msg_ptr (global.get $__str_ret_ptr))
  ...
  (data (i32.const 260) "")
  (data (i32.const 260) "\70\69\6e\67")
)