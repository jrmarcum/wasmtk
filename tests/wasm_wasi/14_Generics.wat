(module
  ...
  (global $__heap_ptr (mut i32) (i32.const 301))
  (global $__str_ret_ptr (mut i32) (i32.const 0))
  (global $__str_ret_len (mut i32) (i32.const 0))
  (global $listHead (mut f64) (f64.const -1))
  (global $listTail (mut f64) (f64.const -1))
  (global $listNodes (mut i32) (i32.const 0))
  ...
  (func $slicesIndexStr (param $s i32) (param $v_ptr i32) (param $v_len i32) (result f64)
    (local $i f64)
    (local.set $i (f64.const 0))
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (f64.lt (local.get $i) (f64.convert_i32_s (i32.load (local.get $s))))))
        (block $cont_0
          (if (i32.eqz (call $__str_cmp (i32.load (i32.add (i32.add (local.get $s) (i32.const 8)) (i32.shl (i32.trunc_f64_s (local.get $i)) (i32.const 3)))) (i32.load offset=4 (i32.add (i32.add (local.get $s) (i32.const 8)) (i32.shl (i32.trunc_f64_s (local.get $i)) (i32.const 3)))) (local.get $v_ptr) (local.get $v_len)))
            (then
            (return (local.get $i))
            )
          )
  ...
  (func $listPush (param $v f64) 
    (local $node i32)
    (local $idx f64)
    (local $__iface_tmp i32)
    (local.set $node (call $__malloc (i32.const 24)))
      (f64.store offset=0 (local.get $node) (local.get $v))
      (i32.store offset=8 (local.get $node) (i32.const 0))
      (f64.store offset=16 (local.get $node) (f64.const -1))
    (global.set $listNodes (call $__dynarr_push_i32 (global.get $listNodes) (local.get $node)))
    (local.set $idx (f64.sub (f64.convert_i32_s (i32.load (global.get $listNodes))) (f64.const 1)))
    (if (f64.eq (global.get $listTail) (f64.const -1))
      (then
      (global.set $listHead (local.get $idx))
      (global.set $listTail (local.get $idx))
      )
      (else
      (i32.store (i32.add (i32.load (i32.add (i32.add (global.get $listNodes) (i32.const 8)) (i32.shl (i32.trunc_f64_s (global.get $listTail)) (i32.const 2)))) (i32.const 8)) (i32.const 1))
      (f64.store (i32.add (i32.load (i32.add (i32.add (global.get $listNodes) (i32.const 8)) (i32.shl (i32.trunc_f64_s (global.get $listTail)) (i32.const 2)))) (i32.const 16)) (local.get $idx))
      (global.set $listTail (local.get $idx))
      )
    )
  )

  (func $listAllElements  (result i32)
    (local $result i32)
    (local $cur f64)
    (local $__iface_tmp i32)
    ...
    (local.set $cur (global.get $listHead))
    (block $break_1
      (loop $loop_1
        (br_if $break_1 (i32.eqz (f64.ne (local.get $cur) (f64.const -1))))
        (block $cont_1
          (local.set $result (call $__dynarr_push_f64 (local.get $result) (f64.load (i32.add (i32.load (i32.add (i32.add (global.get $listNodes) (i32.const 8)) (i32.shl (i32.trunc_f64_s (local.get $cur)) (i32.const 2)))) (i32.const 0)))))
          (local.set $cur (if (result f64) (i32.load (i32.add (i32.load (i32.add (i32.add (global.get $listNodes) (i32.const 8)) (i32.shl (i32.trunc_f64_s (local.get $cur)) (i32.const 2)))) (i32.const 8))) (then (f64.load (i32.add (i32.load (i32.add (i32.add (global.get $listNodes) (i32.const 8)) (i32.shl (i32.trunc_f64_s (local.get $cur)) (i32.const 2)))) (i32.const 16)))) (else (f64.const -1))))
        )
        (br $loop_1)
      )
    )
  ...
  (func $_start (export "_start")
    (local $sl i32)
    (local $__iface_tmp i32)
    (global.set $listNodes (call $__malloc (i32.const 40)))
      (i32.store (global.get $listNodes) (i32.const 0))
      (i32.store offset=4 (global.get $listNodes) (i32.const 8))
    (local.set $sl (i32.const 269))
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (i32.store8 (i32.const 141) (i32.const 122))
          ...
          (i32.store (i32.const 4) (i32.add (i32.const 14) (call $__f64_to_str (call $slicesIndexStr (local.get $sl) (i32.const 266) (i32.const 3)) (i32.const 146))))
          ...
          (drop (call $fd_write
            ...
            (i32.const 1)
            (i32.const 128)))
    (call $listPush (f64.const 10))
    (call $listPush (f64.const 13))
    (call $listPush (f64.const 23))
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (i32.const 0))
          ...
          (i32.store8 (i32.const 137) (i32.const 32))
          (call $numArrStr (call $listAllElements))
  ...
  (data (i32.const 260) "\66\6f\6f")
  (data (i32.const 263) "\62\61\72")
  (data (i32.const 266) "\7a\6f\6f")
  (data (i32.const 269) "\03\00\00\00\03\00\00\00\04\01\00\00\03\00\00\00\07\01\00\00\03\00\00\00\0a\01\00\00\03\00\00\00")
)