(module
  ...
  (func $safeDivide (param $a i32) (param $b i32) (result i32)
    ...
    (try
      (do
        (local.set $result (call $divide (local.get $a) (local.get $b)))
      )
      (catch $__exn_tag
        (local.set $e_len)
        (local.set $e_ptr)
          ...
          (call $__str_gather (local.get $e_ptr) (local.get $e_len) (i32.const 132))
          (i32.store (i32.const 4) (i32.add (i32.const 0) (local.get $e_len)))
  ...
  (func $testThrowString  
    (local $msg_ptr i32)
    (local $msg_len i32)
    (local $e_ptr i32)
    (local $e_len i32)
    (local $__iface_tmp i32)
    (local.set $msg_ptr (i32.const 300))
      (local.set $msg_len (i32.const 12))
    (try
      (do
        (call $throwString (local.get $msg_ptr) (local.get $msg_len))
      )
      (catch $__exn_tag
        (local.set $e_len)
        (local.set $e_ptr)
    ...
    (local.set $r1 (call $safeDivide (i32.const 10) (i32.const 2)))
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (drop (call $fd_write
            ...
            (i32.const 1)
            (i32.const 128)))
    (local.set $r2 (call $safeDivide (i32.const 10) (i32.const 0)))
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (drop (call $fd_write
            ...
            (i32.const 1)
            (i32.const 128)))
    (call $withFinally (i32.const 1))
    (call $withFinally (i32.const -1))
    (call $testThrowString )
    (call $proc_exit (i32.const 0))
  )
  (data (i32.const 260) "\44\69\76\69\73\69\6f\6e\20\62\79\20\7a\65\72\6f")
  (data (i32.const 276) "\6e\65\67\61\74\69\76\65")
  (data (i32.const 284) "\73\75\63\63\65\73\73\0a")
  (data (i32.const 292) "\66\69\6e\61\6c\6c\79\0a")
  (data (i32.const 300) "\63\75\73\74\6f\6d\20\65\72\72\6f\72")
)