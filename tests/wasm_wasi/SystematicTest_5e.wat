(module
  ...
  (func $createMultiplier__inner (param $y i32) (param $x i32) (result i32)
    (return (i32.mul (local.get $x) (local.get $y)))
  )

  (func $createMultiplier (param $x i32) (result i32)
    (local $__closure_ptr i32)
    (local.set $__closure_ptr (call $__malloc (i32.const 8)))
    (i32.store (local.get $__closure_ptr) (i32.const 0))
    (i32.store offset=4 (local.get $__closure_ptr) (local.get $x))
    (local.get $__closure_ptr)
  )

  (func $createMultiplier__trampoline (param $__closure_ptr i32) (param $y i32) (result i32)
    (local $__cap_x i32)
    (local.set $__cap_x (i32.load offset=4 (local.get $__closure_ptr)))
    (call $createMultiplier__inner (local.get $y) (local.get $__cap_x))
  )
  ...
  (table 1 funcref)
  (elem (i32.const 0) $createMultiplier__trampoline)
)