(module
  ...
  (func $createManager (export "createManager") (param $initialRows i32) (result f64)
    (local $data i32)
    (local $summary i32)
    (local $i i32)
    ...
    (;; (vals: i32[]) => {;)
    (;; data.push(vals);;)
    (return (;? data.length ;) (f64.const 0))
    (;; },;)
    (;; () => {;)
    ...
    (local.set $i (i32.const 0))
    (block $break_getSummary
      (loop $loop_getSummary
        (br_if $break_getSummary (i32.eqz (i32.lt_s (local.get $i) (;? data.length ;) (i32.const 0))))
        (block $cont_getSummary
          (local.set $summary (call $__dynarr_push_i32 (local.get $summary) (;? data[i].length ;) (i32.const 0)))
        )
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br $loop_getSummary)
      )
  ...
  (func $_start (export "_start")  
    (local $manager f64)
    (local $stats f64)
    (local.set $manager (call $createManager (;? [[1, 2], [3, 4, 5]] ;) (i32.const 0)))
    (;; manager.addRow([6]);;)
    (local.set $stats (;? manager.getSummary() ;) (f64.const 0))
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__f64_to_str (;? stats.length ;) (f64.const 0) (i32.const 132)))
