(module
  ...
  (func $testArrayRest (export "testArrayRest")  (result i32)
    (local $nums i32)
    (local $_first i32)
    (local $rest i32)
    (local.set $nums (i32.const 260))
    (local.set $_first (i32.load (i32.const 260)))
      (local.set $rest (call $__malloc (i32.const 40)))
      (i32.store (local.get $rest) (i32.const 2))
      (i32.store offset=4 (local.get $rest) (i32.const 8))
      (i32.store offset=8 (local.get $rest) (i32.load (i32.const 264)))
      (i32.store offset=12 (local.get $rest) (i32.load (i32.const 268)))
    (return (i32.load (local.get $rest)))
  )
  (func $_start (export "_start")
    (local $__iface_tmp i32)
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__i32_to_str (call $testArrayRest) (i32.const 132)))
