(module
  ...
  (global $__heap_ptr (mut i32) (i32.const 267))
  ...
  (func $rev (param $s_ptr i32) (param $s_len i32) 
    (local $out_ptr i32)
    (local $out_len i32)
    (local $i i32)
    ...
    (local.set $out_ptr (i32.const 264))
      (local.set $out_len (i32.const 0))
    (local.set $i (i32.const 0))
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (i32.lt_s (local.get $i) (local.get $s_len))))
        (block $cont_0
          (local.set $out_ptr (local.get $out_ptr))
      (local.set $out_len (local.get $out_len))
      (local.set $__str_op_ptr (call $__malloc (i32.const 1)))
      (i32.store8 (local.get $__str_op_ptr) (call $__str_char_code_at (local.get $s_ptr) (local.get $s_len) (local.get $i)))
      (call $__str_concat (local.get $out_ptr) (local.get $out_len) (local.get $__str_op_ptr) (i32.const 1))
      (local.set $out_len)
      (local.set $out_ptr)
        )
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
  ...
  (func $_start (export "_start")
    ...
    (local $__str_op_ptr i32)
    (local $__str_op_len i32)
    (local.set $chars_ptr (i32.const 260))
      (local.set $chars_len (i32.const 4))
    (local.set $result_ptr (i32.const 264))
      (local.set $result_len (i32.const 0))
    (local.set $result_ptr (local.get $result_ptr))
      (local.set $result_len (local.get $result_len))
      (local.set $__str_op_ptr (call $__malloc (i32.const 1)))
      (i32.store8 (local.get $__str_op_ptr) (call $__str_char_code_at (local.get $chars_ptr) (local.get $chars_len) (i32.const 2)))
    ...
    (local.set $result_ptr (local.get $result_ptr))
      (local.set $result_len (local.get $result_len))
      (local.set $__str_op_ptr (call $__malloc (i32.const 1)))
      (i32.store8 (local.get $__str_op_ptr) (call $__str_char_code_at (local.get $chars_ptr) (local.get $chars_len) (i32.const 0)))
      (call $__str_concat (local.get $result_ptr) (local.get $result_len) (local.get $__str_op_ptr) (i32.const 1))
      (local.set $result_len)
      (local.set $result_ptr)
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (i32.const 0))
          (call $__str_gather (local.get $result_ptr) (local.get $result_len) (i32.const 132))
          (i32.store (i32.const 4) (i32.add (i32.const 0) (local.get $result_len)))
        ...
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (i32.const 0))
          (call $rev (i32.const 264) (i32.const 3))
  ...
  (data (i32.const 260) "\41\42\43\44")
  (data (i32.const 264) "")
  (data (i32.const 264) "\78\79\7a")
)