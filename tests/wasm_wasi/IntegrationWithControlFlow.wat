(module
  ...
  (func $testArrowInLoop (export "testArrowInLoop") (param $limit i32) (result i32)
    (local $checker i32)
    (local $count i32)
    (local $i i32)
    (local.set $checker (i32.const 0))
    (local.set $count (i32.const 0))
    (local.set $i (i32.const 0))
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (i32.lt_s (local.get $i) (local.get $limit))))
        (block $cont_0
          (if (call $checker (local.get $i))
            (then
  ...
  (func $checker (param $n i32) (result i32)
