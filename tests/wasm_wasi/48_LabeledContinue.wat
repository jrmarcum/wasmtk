(module
  ...
  (func $main  
    ...
    (block $break_outer
      (loop $loop_outer
        (br_if $break_outer (i32.eqz (i32.lt_s (local.get $i) (i32.const 5))))
        (block $cont_outer
          (local.set $j (i32.const 0))
          (block $break_0
            (loop $loop_0
              (br_if $break_0 (i32.eqz (i32.lt_s (local.get $j) (i32.const 5))))
              (block $cont_0
    ...
    (local.set $outer2 (i32.const 0))
    (block $break_loop
      (loop $loop_loop
        (br_if $break_loop (i32.eqz (i32.lt_s (local.get $outer2) (i32.const 4))))
        (block $cont_loop
          (local.set $outer2 (i32.add (local.get $outer2) (i32.const 1)))
          (local.set $inner (i32.const 0))
          (block $break_1
            (loop $loop_1
              (br_if $break_1 (i32.eqz (i32.lt_s (local.get $inner) (i32.const 4))))
              (block $cont_1
                (local.set $inner (i32.add (local.get $inner) (i32.const 1)))
                (if (i32.eq (local.get $inner) (i32.const 2))
                  (then
                  (br $cont_loop)
                  )
                )
