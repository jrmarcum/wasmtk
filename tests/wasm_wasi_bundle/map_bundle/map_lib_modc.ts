// Tier-1 stdlib capability — Map<i32, i32> (open-addressing hash table) as a modc library.
//
...
// resolve to the host module's shared bump cursor, so the Map lives on one shared heap.
//
// This reuses the Set<i32> hash core (same linear-probing + ×2 grow) and adds a parallel
// values array so each occupied bucket holds (key, value).
//
// A "map handle" is an i32 pointer to a 5-slot Int32Array header:
//   [0] count    — number of distinct keys
//   [1] cap       — bucket capacity (power of two)
//   [2] keysPtr   — pointer to an Int32Array(cap) of bucket keys
//   [3] valsPtr   — pointer to an Int32Array(cap) of bucket values (parallel to keys)
//   [4] usedPtr   — pointer to an Int32Array(cap) of 0/1 occupancy flags
...
/** Create an empty map; returns the handle (header pointer). */
/** @export */
export function mapNew(): i32 {
  const header: Int32Array = new Int32Array(5);
  header[0] = 0;                       // count
  header[1] = INITIAL_CAP;             // cap
  header[2] = allocSlots(INITIAL_CAP); // keysPtr
  header[3] = allocSlots(INITIAL_CAP); // valsPtr
  header[4] = allocSlots(INITIAL_CAP); // usedPtr
  return header as unknown as i32;
}

/** Double capacity and rehash every occupied bucket. The handle is unchanged. */
function mapGrow(h: i32): void {
  ...
  const oldVals: Int32Array = header[3] as unknown as Int32Array;
  const oldUsed: Int32Array = header[4] as unknown as Int32Array;

  const newCap: i32 = oldCap * 2;
  const newKeysPtr: i32 = allocSlots(newCap);
  const newValsPtr: i32 = allocSlots(newCap);
  const newUsedPtr: i32 = allocSlots(newCap);
  const newKeys: Int32Array = newKeysPtr as unknown as Int32Array;
  const newVals: Int32Array = newValsPtr as unknown as Int32Array;
...
/** @export */
export function mapSet(h: i32, key: i32, value: i32): void {
  ...
  const vals: Int32Array = header[3] as unknown as Int32Array;
  const used: Int32Array = header[4] as unknown as Int32Array;
  const mask: i32 = cap - 1;

  let idx: i32 = key & mask;
  while (used[idx] !== 0) {
    if (keys[idx] === key) {
      vals[idx] = value; // update existing key
      return;
    }
    idx = (idx + 1) & mask;
  }
...
/** @export */
export function mapGet(h: i32, key: i32, fallback: i32): i32 {
  const header: Int32Array = h as unknown as Int32Array;
  const cap: i32 = header[1];
  const keys: Int32Array = header[2] as unknown as Int32Array;
  const vals: Int32Array = header[3] as unknown as Int32Array;
...
/** Return 1 if `key` is in the map, else 0. */
/** @export */
export function mapHas(h: i32, key: i32): i32 {
  const header: Int32Array = h as unknown as Int32Array;
  const cap: i32 = header[1];
  const keys: Int32Array = header[2] as unknown as Int32Array;
  const used: Int32Array = header[4] as unknown as Int32Array;
...
/** @export */
export function mapSize(h: i32): i32 {
  const header: Int32Array = h as unknown as Int32Array;
  return header[0];
}
