(module
  ...
  (func $makeBufferA (export "makeBufferA") (param $size i32) (result i32)
    (local $buf i32)
    (local $i i32)
    (local $__iface_tmp i32)
    (local.set $buf (call $__malloc (i32.const 40)))
      (i32.store (local.get $buf) (i32.const 0))
      (i32.store offset=4 (local.get $buf) (i32.const 8))
    (local.set $i (i32.const 0))
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (i32.lt_s (local.get $i) (local.get $size))))
        (block $cont_0
          (local.set $buf (call $__dynarr_push_i32 (local.get $buf) (i32.add (i32.const 170) (local.get $i))))
          (local.set $i (i32.add (local.get $i) (i32.const 1)))
        )
        (br $loop_0)
      )
    )
    (return (i32.load (local.get $buf)))
  )
)