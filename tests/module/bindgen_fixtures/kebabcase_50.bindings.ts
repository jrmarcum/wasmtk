...
export interface ModuleExports {
  readId(x: number): number;
  toHtml(x: number): number;
}
...
export async function loadModule(
  ...
  return {
    readId(x: number): number { return (_ex("readId") as (...a: unknown[]) => unknown)(x) as number; },
    toHtml(x: number): number { return (_ex("toHtml") as (...a: unknown[]) => unknown)(x) as number; },
  };
}
