 ...
 */ export function handlePromptSelect(message, indicator, values, clear, visibleLinesInit, fitToRemainingHeight, valueChange, handleInput) {
  ...
  let visibleLines = visibleLinesInit ?? Math.min(availableHeight, values.length);
  while(true){
    ...
    const filteredChunks = indexedValues.filter((item)=>{
      if (searchBuffer === "") {
        ...
        return (typeof item.value === "string" ? item.value : item.value.label).toLowerCase().includes(searchBuffer.toLowerCase());
      }
    });
    ...
    if (hasUpArrow) {
      output.writeSync(encoder.encode(`${ARROW_PADDING}${MORE_CONTENT_BEFORE_INDICATOR}\r\n`));
    }
    for (const [index, { absoluteIndex, value }] of visibleChunks.entries()){
      ...
      output.writeSync(encoder.encode(`${start}${maybePrefix ? ` ${maybePrefix}` : ""} ${typeof value === "string" ? value : value.label}\r\n`));
    }
    if (hasDownArrow) {
      output.writeSync(encoder.encode(`${ARROW_PADDING}${MORE_CONTENT_AFTER_INDICATOR}\r\n`));
    }
    const n = input.readSync(buffer);
    if (n === null || n === 0) break;
    const string = decoder.decode(buffer.slice(0, n));
    const processedInput = handleInput(string, filteredChunks[activeIndex]?.absoluteIndex, {
      etx: ()=>{
    ...
    if (fitToRemainingHeight) {
      availableHeight = Math.min(availableHeight, Deno.consoleSize().rows - SAFE_PADDING);
    } else {
    ...
    (hasUpArrow ? 1 : 0) + length + (hasDownArrow ? 1 : 0);
...
//# sourceMappingURL=_prompt_select.js.map