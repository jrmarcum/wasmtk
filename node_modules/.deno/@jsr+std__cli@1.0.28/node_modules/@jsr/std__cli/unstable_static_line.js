...
import * as Ansi from "./unstable_ansi.js";
let nextID = 0;
const linesAtTop = [];
const linesAtBottom = [];
const getRows = "Deno" in globalThis ? ()=>globalThis.Deno.consoleSize().rows : ()=>globalThis.process.stderr.rows;
const write = "Deno" in globalThis ? (()=>{
  const encoder = new TextEncoder();
  return (x)=>// deno-lint-ignore no-explicit-any
    globalThis.Deno.stderr.write(encoder.encode(x));
})() : (x)=>globalThis.process.stderr.write(x);
/**
 ...
 */ export class StaticLine {
   ...
   */ constructor(on = "bottom"){
    ...
    write((this.#atTop ? "" : Ansi.shiftUpAndInsert()) + Ansi.setScrollableRegion(top, bottom) + Ansi.setCursorPosition(bottom));
  }
  /**
   ...
   */ async write(line) {
    ...
    await write(Ansi.SAVE_CURSOR + Ansi.DISABLE_ORIGIN_MODE + Ansi.DISABLE_AUTO_WRAP + Ansi.setCursorPosition(this.#atTop ? linesAtTop.indexOf(this.#id) + 1 : getRows() - linesAtBottom.indexOf(this.#id)) + Ansi.ERASE_LINE + line + Ansi.RESTORE_CURSOR);
  }
  /**
   ...
   */ async releaseLine() {
    ...
    await write(Ansi.SAVE_CURSOR + (this.#atTop ? top - index ? Ansi.setScrollableRegion(index + 1, top + 1) + Ansi.shiftUpAndInsert() : Ansi.setCursorPosition(index + 1) + Ansi.ERASE_LINE : rows - index - bottom ? Ansi.setScrollableRegion(bottom, rows - index) + Ansi.shiftDownAndInsert() : Ansi.setCursorPosition(rows) + Ansi.ERASE_LINE) + Ansi.setScrollableRegion(top, bottom) + Ansi.RESTORE_CURSOR);
  }
}
//# sourceMappingURL=unstable_static_line.js.map