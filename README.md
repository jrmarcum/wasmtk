...
| 42 | WASI p2 component wrapping (`wasmtk componentize`) | **`componentNew`** from `@bytecodealliance/jco` (npm dependency, no separate install) wraps a wasic-compiled core WASM module as a WebAssembly Component Model binary. **WASI detection:** `detectWasiImports()` scans the binary for `wasi_snapshot_preview1` and attaches the p1→p2 command adapter when found — pure `modc` library modules are wrapped with no adapter. **Self-describing output:** `componentWit()` extracts the interface encoded in the component binary and writes it as `<name>.component.wit` alongside the component — any Component Model consumer (wasmtime, wit-bindgen) can read the interface without a separate file. **WIT string fix:** `watTypeToWit("string")` now returns WIT-native `"string"` instead of `"s32"`, making generated WIT files compatible with jco and wit-bindgen; string params in `generateWit()` emit a single `name: string` entry instead of two `ptr: s32 / len: s32` entries. **`main.ts`:** `componentize` case compiles `.ts` input first if needed; `--name` sets the output `.component.wasm` path. **`deno.json`:** `@bytecodealliance/jco` added to imports; `"./componentize"` added to exports. |
...
Phases 43–47 are planned in the following order. Each builds on the previous — Phase 43 (WIT diff) and Phase 44 (manifest) are prerequisites for Phase 45 (upgrade command). Phase 46 (partitioner) and Phase 47 (browser bundle) are independent of the upgrade pipeline.

#### Phase 43 — WIT Compatibility Checker (`wasmtk compat`) 🔜

Compares two `.wit` files or two `.wasm` modules and reports interface changes. Outputs three categories: `ADDED` (new exports, safe), `CHANGED` (same name, different signature — breaking), `REMOVED` (deleted exports — breaking). Used as the gating step before a library upgrade.

```text
wasmtk compat mathlib.v1.wit mathlib.v2.wit
  ADDED    sqrt: func(x: f64) -> f64
  CHANGED  add: func(a: s32, b: s32) -> s32  →  func(a: s64, b: s64) -> s64  ⚠ breaking
  REMOVED  legacy_mul: func(a: s32, b: s32) -> s32  ⚠ breaking
```

No runtime required. Pure WIT text parse + diff.

---

#### Phase 44 — WASM Library Manifest (`wasm.json`) 🔜

Introduces a `wasm.json` project file that declares WASM library dependencies — similar to `package.json` but for compiled WASM modules.
...
```json
{
  "libs": [
    {
      "name": "mathlib",
      "source": "src/wasm/mathlib.ts",
      "output": "src/wasm/mathlib.wasm",
      "wit":    "src/wasm/mathlib.wit",
      "version": "1.0.0"
    }
  ]
}
...
`wasmtk init` creates a starter `wasm.json`. `wasmtk build` recompiles all stale entries. The manifest is the single source of truth for the upgrade pipeline.

---

#### Phase 45 — WASM Library Upgrade (`wasmtk upgrade`) 🔜

Recompiles a library entry from source, runs `wasmtk compat` against the stored WIT snapshot, and replaces the `.wasm` only when the diff reports no breaking changes.

```text
wasmtk upgrade mathlib
  Compiling mathlib...
  Checking compatibility...
  ADDED    sqrt: func(x: f64) -> f64   ✅ safe
  Upgrade complete: src/wasm/mathlib.wasm
```

If breaking changes are detected, the command exits non-zero and leaves the existing `.wasm` untouched. Supports `--force` to override the compatibility gate.

---

#### Phase 46 — TypeScript Hybrid Partitioner (`wasmtk partition`) 🔜

Analyses a TypeScript source file and automatically splits it into two outputs:

- A **WASM module** (`.wasm`) containing all functions that use only wasic-supported constructs (pure arithmetic, strings, arrays, structs, etc.)
- A **TypeScript host file** (`main.ts`) containing the JS-runtime-dependent portions (`fetch`, `JSON.parse`, DOM, `async/await`, etc.) that import and call the WASM module.

The partitioner uses a feature scan + call-graph propagation pass: any function that calls an impure function is itself considered impure and routed to the host file. The result is a hybrid program where the CPU-bound logic runs in WASM at near-native speed and the I/O logic remains in TypeScript.

---

#### Phase 47 — Browser Bundle (`wasmtk browser-bundle`) 🔜

Produces a browser-ready importable JavaScript module from a compiled WASM file. Uses `jco transpile` under the hood to generate a `.js` loader + `.d.ts` type declarations alongside the core `.wasm`. Intended for external consumers (library authors, browser targets) where a TypeScript import is not possible.

```text
wasmtk browser-bundle mylib.wasm --out dist/
  dist/mylib.js       (ES module loader)
  dist/mylib.d.ts     (TypeScript declarations)
  dist/mylib.wasm     (core WASM, unchanged)
```

Note: TypeScript consumers using `@jrmarcum/universal-wasm-loader` do not need this command — they can import the `.wasm` file directly.
