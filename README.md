...
> **Resolved (2026-08-25) — a full-corpus directory run now completes in one process.** This
> previously died with `Fatal JavaScript out of memory`, and `proposals/custom-descriptors/exact.wast`
> exhausted the heap on its own. Both causes are fixed: an infinite loop in the runner's own S-expr
...
> a single process: **37,365 passed, 100 failed, 27,153 skipped**, no OOM.
...
| ✅ | Core bug fix (2026-08-24) | `console.log` output was silently TRUNCATED on Wasmtime | **Problem:** WASI permits `fd_write` to write fewer bytes than requested and report the true count in `*nwritten`; Wasmtime does exactly that (measured: it writes `iov[0]` only and reports `nwritten = 2` of 3, which is conforming). Every emitted call was `(drop (call $fd_write …))` — the result and `nwritten` discarded, no retry — so any output split across more than one iovec was silently lost. The visible symptom was `console.log(<bool expr>)` losing its trailing newline: `1_values` printed `falsetruefalse`, exactly 3 bytes short. **37 corpus modules were affected**, and every one of them was green on V8, which accepts the same bytes. **Fix:** a `$__fd_write_all` helper that loops until every iovec is drained, advancing `ptr` and shrinking `len` in place on a partial write, bailing on a non-zero errno and on zero progress (a runtime reporting success while writing nothing would otherwise spin forever). Both emission sites rerouted, including the gather path, which showed no symptom but carried the identical latent defect — one iovec makes a short write less likely, never impossible. The `dync` `__host_print` path got its own inline loop, since it is spliced into an already-merged module that need not carry the shared helper. Found by the new multi-engine gate on its first run; covered by `tests/engine_cross_check_tests.ts` |
| ✅ | Core bug fix (2026-08-24) | `console.log(a == b)` produced an INVALID module and reported success | **Problem:** the boolean-argument detector tested `/===\|!==/` but never loose `==` / `!=`, so `console.log(a == b)` was not recognised as a boolean, fell through to the numeric path, and emitted an i32 comparison where an f64 was expected — `CompileError: expected type f64, found f64.eq of type i32`. wasic printed `✅ WASI … (3136 bytes)` first, so the failure landed at the user's runtime rather than at compile time. `wasic.ts` handles `==` correctly everywhere else (`if (a == b)`, `const c: boolean = a != b`) — this was purely the `console_log.ts` half of the parallel-operator pair. **Fix:** added `==` / `!=` to the top-level operator scan. Verified across V8, Wasmtime, Wasmer and wazero |
| ✅ | Core bug fix (2026-08-24) | `console.log(<bool expr>)` evaluated its argument TWICE | **Problem:** a boolean segment emitted two value-form `(if (result i32) <EXPR> …)` stores — one for the pointer, one for the length — interpolating the operand into both. For a variable that is harmless; for a **call** returning `boolean` it ran the call twice, so `console.log(isPositive(5))` invoked `isPositive` twice and any side effect happened twice. That is a semantic change, not a formatting one. The gather path was worse still, at three evaluations. **Fix:** a single statement-form `if` with both stores inside each arm, so the operand is evaluated exactly once. No temp local was needed. That also removed the reason `boolexpr` was excluded from gather mode, so `console.log(x > 0)` now absorbs its trailing newline into one iovec instead of two — the exclusion and the bug were one knot, not two |
...
| ✅ | 18 bug fix (2026-08-25) | Merged-module data relocation silently disabled by a WAT printer change | wabt-ts `1.4.1` prints constant expressions **unfolded** — `(global $g (mut i32) i32.const 0)` — where earlier versions printed them folded, `(global $g (mut i32) (i32.const 0))`. Both are valid WAT. Six regexes across `wasmmerge.ts`, `wasmbundle.ts` and `wasic.ts` required the folded bracketing and therefore matched **nothing** rather than failing: `getDataMaxEnd` returned `0` so `dataOffset` never advanced and every merged module's data stacked at the same base; the data-extent scan left `dataHi === 0`, whose guard disables static-pointer relocation wholesale; and `wasic` fell through to its `260` data-base fallback. The merged module's `$__heap_ptr` was consequently seated **inside static data**, so the shared bump allocator handed out addresses over live constants — surfacing not as a crash but as an **infinite loop** in bundled dynrt modules. All six sites now read constant expressions through `constExprValue` / `replaceConstExpr`, which accept either bracketing and preserve the one wabt used; verified by rebuilding the same module against both backend versions and diffing, with **zero numeric differences**. Covered by `wasi_tests` (`dynrt_eval_bundle/main_func`, `main_funcs`), `bundle_tests`, `merge_tests`, `wasmmerge_guard_tests` |
| ✅ | spec runner bug fix (2026-09-19) | `wasmtk wast` — `assert_malformed` now means a DECODE failure, not any failure | The `.wast` runner treated every assembly failure as satisfying `assert_malformed`, so an **encoder** error — or a module the host rejected as *invalid* — counted as "this text cannot be decoded". Assembly stages are now distinguished: only a genuine parse failure passes; a module that parses and then fails to encode, or that the host rejects as invalid, is reported as a SKIP naming which stage it reached. `(module binary …)` forms are unaffected, since there a decoder rejection IS the asserted failure. Two assertions across the 288-file corpus were passing for the wrong reason and are now honestly reported, and both turned out to be real toolchain divergences worth reporting upstream. Covered by `wast_tests` |
| ✅ | spec runner enhancement (2026-08-27) | `wasmtk wast` marshals `ref.null` across the JS boundary | The `.wast` runner could not pass or compare a **null reference**, so every assertion using `ref.null` was skipped regardless of what the assembler could do — `ref_null.wast` sat at 0 passed / 32 skipped. `ref.null <heaptype>` is now marshalled as JS `null` for both arguments and expected results. **+123 assertions across 16 files with zero regressions**, and two files shed failures: `ref_null` 0 → 25, `table_fill64` 9 → 41, `table_fill` 9 → 25, `table_set` 11 → 21, `table_set64` 4 → 14, plus `elem`, `global`, `select`, `table`, `extern`, `return_call`, `ref_is_null`, `table_get*`. Corpus total **37,247 → 37,370 passing assertions**. `ref.func` / `ref.extern N` remain unsupported — those denote host references with identity, not a literal conversion — and heap types V8 cannot marshal (`exnref`, `anyref`, user-defined) are reported as SKIPS rather than failures, because that is a limit of the JS harness and not a toolchain defect. Covered by `wast_tests` |
| ✅ | bug fix (2026-08-27) | Windows file-write race on `.wasm` output | A just-exited child process, an indexer or a virus scanner can still hold a handle to a file wasmtk has finished with, so a write could fail with `os error 32` ("being used by another process") — intermittently, and never as a real error. `rt.writeFile` now retries a Windows sharing violation five times over ~1.2s; **every other failure still propagates on the first attempt**, so this is a race fix and not error-swallowing. Affects every producer, not just Go. Covered by `go_asyncify_tests` |
...
| ✅ | 22 bug fix (2026-08-24) | An enum initializer that could not be evaluated silently became `0` | **Problem:** `evalEnumExpr` caught any evaluation error and returned `0`, so `enum E { A = SomethingUndefined }` compiled with `A = 0` — **silently colliding** with any other member at 0, with nothing in the output saying so. Comparisons against the enum then went wrong at runtime. A non-finite result (`NaN`) also fell through `\| 0` to the same 0. **Fix:** both cases now record a `diagnostics` entry, which aborts the compile with the supported grammar named (prior members, integer literals, parens, and `\| & ^ << >> >>> + - * / %`). Converting silent-wrong into a hard abort, per the project's standing preference |
...
| ✅ | bug fix (2026-08-24) | `wasmtk convert` / `wasmtk wasm2js` printed an error but EXITED 0 | **Problem:** both were `Promise<void>` — the catch logged `❌ Conversion failed: …` and returned, and the CLI fell straight through to `break`, so the process exited **0**. Any script or CI checking `$?` read a failed conversion as success. **Fix:** both now return `Promise<boolean>` and the CLI exits 1 on failure. Isolated rather than CLI-wide — `wasmtk wasic` on bad input already exited 1 correctly, which is what identified this as a local hole. Verified in both directions: broken input exits 1, good input exits 0 |
...
| ✅ | 32 bug fix (2026-07-30) | A union field shared by several variants is resolved, not dropped | The flat super-struct builder skipped any field name it had already seen, so a field declared in more than one variant silently took the **first** variant's type. With `type V = { type: "intVal"; val: i32 } \| { type: "floatVal"; val: f64 }`, `val` was laid out as a 4-byte `i32`: the literal `25.5` was stored as the integer `25`, and reading it back in the narrowed `floatVal` branch emitted `i32.load` inside a function declared `(result f64)` — invalid WASM, rejected at instantiate time. Shared fields are now resolved across **all** variants before layout and widened to the type that can represent every one of them (`i32`/`f32` → `f64`, `i32` → `i64`), which is what TypeScript already means here, since `i32` and `f64` are both `number` aliases. Fields declared after a widened slot keep their natural alignment, and widening is order-independent. A collision that is **not** a numeric widening (e.g. `string` vs `f64`) is now a hard compile error naming the union, the field and both types, instead of the opaque `Offset is outside the bounds of the DataView` it used to produce. Stress tests `19_DiscUnionSuperStructLayout`, `19_SwitchCaseVariantDispatch`, `19_ElseIfNarrowingFieldCast`; regression `19_UnionSharedFieldWidening` |
...
| ✅ | 33 bug fix (2026-07-30) | Passing an intersection to a base-typed parameter is now checked, not assumed | The row above notes that handing a `A & B` value to a function expecting `A` is safe "because the first N bytes of the derived layout are identical to the base layout" — true only when `A` is the intersection's **first** constituent, and nothing enforced it. `type Sprite = Renderable & Transform` passed to `getScaleArea(t: Transform)` read `alpha`/`scaleX` where it wanted `scaleX`/`scaleY` and printed `1.6` instead of `6` — every offset a valid in-bounds slot, so no trap and no error, just wrong numbers. Structs are passed by pointer and the callee reads fields at its own parameter type's offsets, so wasic now verifies at every call that the parameter's struct layout is a byte-exact **prefix** of the argument's (same field name, offset, type and size) and otherwise **aborts the compile** with a diagnostic naming the variable, both types, the offending field, both byte offsets, and the two fixes: list the base constituent first (`type Sprite = Transform & Renderable`) or type the parameter with the intersection. The check runs on all three call-emission paths, including calls nested inside a `console.log` argument. Stress tests `33_IntersectionMixedTypeMerge`, `33_ChainedIntersectionIntDivide`, `33_IntersectionBasePointerParam`; regressions `33_IntersectionBasePrefixGuard` (rejected shape) + `33_IntersectionPrefixOk` (four shapes that must still compile) |
...
| ✅ | 34 bug fix (2026-07-30) | A predicate whose target is an inline object type now parses, and narrows to a real layout | The row above describes the `param is Type` annotation, and only a **named** `Type` was ever accepted. Writing the target as an inline object type — `function isSphere(g: Geometry): g is { type: "sphere"; r: f64 }`, the form a discriminated-union variant is normally spelled — made the whole function header fail to match, so the predicate function was **never parsed at all** and every call to it aborted the compile with `Unknown function 'isSphere' — not declared in this module`, advice that points at a typo when the function is present and correct. Inline object types are now accepted in the annotation. Fixing that uncovered a second problem behind it: an inline target names no declared type, so narrowing was silently skipped and the then-branch compiled against the un-narrowed type. For a discriminated union that is harmless — the flat super-struct already carries every variant's fields at their real offsets — but over an interface hierarchy (`s: Shape` narrowed to `{ kind: i32; radius: f64 }`) the field read compiled clean and produced **0** instead of **5**. An inline target is now resolved to the declared type that has exactly those fields, so narrowing uses that type's real offsets; and if no declared type matches **and** the variable's own type is missing a field the target names — the case with no layout to read against — the compile **aborts** with a diagnostic naming the predicate, the variable, the missing fields and the containing type, instead of emitting a wrong number. Stress tests `34_TypePredicateBasicNarrowing`, `34_TypePredicateElseIfChains`, `34_DiscUnionPredicateInlineTarget`; regressions `34_InlinePredicateTargetNarrowing` (the read that returned 0, plus a named-target guard) + `34_InlinePredicateUnresolvable` (the shape that must abort) |
