...
| enhancement (2026-07-01) | `mathlib` `sin`/`cos` rebuilt to fdlibm — V8-parity trig | **Replaces the single-range degree-13 minimax (~1e-11 accurate) with fdlibm.** `src/wasm/mathlib.wat` `$sin`/`$cos` now use **`__kernel_sin`/`__kernel_cos`** (the exact polynomials V8 uses) preceded by a **table-free Veltkamp-split argument reduction** (`$__trig_reduce`): `n = f64.nearest(x·2/π)`, then `r + rt = x − n·(π/2)` accumulated in double-double over the seven fdlibm `PIo2[]` 24-bit chunks. `n` is Veltkamp-split into `nhi` (≤24 significant bits) + `nlo` so every `nhi·PIo2[i]` / `nlo·PIo2[i]` product is **exact** and each subtraction is Sterbenz-exact; a `Fast2Sum` tail preserves the low bits. Result: **≤1 ULP vs V8 for `\|x\| < ~1e12`** (bit-exact on ~97 %; the residual ~3 % 1-ULP is V8's correctly-rounded path), degrading gracefully to ~14 sig-figs at 1e15 — a large jump from the old ~11–13 sig-figs. Deliberately uses **no Payne-Hanek table and no linear memory** (pure f64 arithmetic) so it splices cleanly into the merged module; the `(r, rt)` output travels through two new mathlib globals `$__tr`/`$__trt` (verified to survive the merge and coexist with the RNG global). `Math.tan` stays `sin/cos`. **`Math.sin(5e8)` is now byte-exact with V8**, so `7a_constants` dropped its `// @allow-output-diff`. Full suite 374/374. Regenerate after editing: `wasmtk convert src/wasm/mathlib.wat` then `deno run -A scripts/gen_mathlib_bytes.ts` |
