...
> is green (`tests/wasi/wasm_wasi` 395/395) on `jsr:@jrmarcum/wabt-ts` + `jsr:@jrmarcum/binaryen-ts`,
...
| ✅ | 27 bug fix (2026-07-29) | `console.log` arithmetic starting with a number or `(` failed to compile | `console.log("x:", 1 + n)` and `console.log("x:", (n + 0) * 5)` failed to instantiate for integer variables, while the equivalent `n + 1` worked — a leading literal or parenthesis left the expression mis-typed as floating-point. Both now compile; `1.5 + n` still means floating-point |
