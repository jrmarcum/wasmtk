...
| 34 | Type predicates `x is T` | For Phase 32 tagged unions (complete): emit `(i32.eq (i32.load $x) (i32.const TAG_FOO))`; predicate returns bool |
| 35 | `typeof` (type position) / `keyof T` | `typeof x` resolved to known `WatType` at compile time; `keyof T` yields string-literal union of struct field names (usable with Phase 32 discriminated unions, complete) |
