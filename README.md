...
All planned phases are implementable under WASI Preview 1. Phases 44–45 close the 4 remaining Go-by-Example test gaps (221/226 PASS as of 2026-05-14); Phase 46 addresses a compiler correctness gap (string escape sequence processing).

| Phase | Feature | Target tests | Key changes |
| --- | --- | --- | --- |
| 44 | **Function pointer arrays (`Array<FunctionType>`)** | `defer.ts`, `exit.ts`, `collection-functions.ts` (partially) | `Array<() => void>` (and similar) detected in `findDynamicArrays` pre-pass; stored as a standard dynamic `i32` array of WASM table indices; `arr.push(fn)` resolves `fn` via `getFuncTableIdx`; `arr[i]()` loads the index then `call_indirect` with the matching functype; `arr.length = 0` zeroes the dynamic array length header. |
