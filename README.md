...
| 46 | **String escape sequence processing** | `xml_6b.ts` (workaround in place); any test using `\n`, `\t`, `\\`, `\"`, `\r`, `\xHH`, `\uHHHH` in string or template literals | Add `unescapeString(raw: string): string` utility in `wasic.ts`; call it at all string extraction sites (`allocString()` and template literal static segment extraction); handle `\n`→`0x0a`, `\r`→`0x0d`, `\t`→`0x09`, `\b`→`0x08`, `\f`→`0x0c`, `\v`→`0x0b`, `\\`→`\`, `\"`→`"`, `\'`→`'`, `` \` ``→`` ` ``, `\0`→`0x00`, `\xHH`, `\uHHHH`, `\u{H…}`; apply same fix in `console_log.ts` for `{ kind: "literal", text }` segments; after fix `xml_6b.ts` can revert to its original `buildPlantXml(): string` form. |
| 47 | **Class inheritance** | New: `BasicClassInheritance_47.ts`, `SuperConstructor_47.ts`, `ClassMethodOverride_47.ts`, `VirtualDispatch_47.ts`, `Phase47Combined_47.ts` | Three-stage implementation. **Stage 1** — field layout merging in `parseClasses()`: when `extends BaseName` is detected, prepend parent fields with original offsets before derived fields (identical to Phase 30 interface inheritance); new `classInheritance: Map<string, string>`. **Stage 2** — `super(args)` in derived constructor: detect in `emitStatement`, look up parent `ClassDef`, emit parent constructor body inline with argument substitution against the same `__self` pointer. **Stage 3** — virtual dispatch via class tag: 4-byte tag at instance offset 0 (controlled by `classHeaderSize = 4`, opt-in only when inheritance is present); `classTags: Map<string, number>` assigned post-parse; method calls on base-typed receivers emit a `br_if` tag-dispatch chain; `virtualMethods: Map<string, Set<string>>` tracks which methods are overridden. |
| 48 | **Language completeness: Number API, operators, control flow** | New: `NumberConstants_48.ts`, `NumberPredicates_48.ts`, `SwitchFallthrough_48.ts`, `ObjectDestructDefault_48.ts`, `LabeledContinue_48.ts`, `ExponentAssign_48.ts`, `Phase48Combined_48.ts` | Six independent additions. `Number.NaN`/`POSITIVE_INFINITY`/`NEGATIVE_INFINITY`/`EPSILON`/`MAX_SAFE_INTEGER`/`MIN_SAFE_INTEGER`/`MAX_VALUE`/`MIN_VALUE` → named `(f64.const ...)` lookups. `Number.isNaN(x)` → `(f64.ne x x)`; `Number.isFinite(x)` → infinity range check; `Number.isInteger(x)` → `(f64.eq (f64.floor x) x)`. `**=` compound assignment → reuse existing `**`/`Math.pow` emitter. `switch` fallthrough → omit trailing `(br $switch_end)` when case body ends without `break`/`return`/`throw`. Object destructuring with defaults (`const { x = 0 } = vec`) → emit zero-equality conditional after field load. Labeled `continue` → add `continueLabel` to `controlStack` entries; emit `(br $loop_outerLabel)`. |
| 49 | **Optional chaining and collection method completeness** | New: `OptionalChaining_49.ts`, `StringAt_49.ts`, `ArrayConcat_49.ts`, `ChainedMethods_49.ts`, `Phase49Combined_49.ts` | Four additions. Optional chaining `?.`: non-nullable bases stripped to `.` in source pre-pass; nullable bases (Phase 24 `T \| null` vars) emit `(if (result T) nullFlag (then load) (else default))`. `String.prototype.at(index)` with negative-index wrap: `(call $__str_char_at ptr len (select idx (i32.add len idx) (i32.ge_s idx (i32.const 0))))`. `Array.prototype.concat(other)` → wire to existing `$__dynarr_concat_T` helper used by spread literals. Chained array method calls (`arr.filter(f).map(g)`): when receiver of a method is itself a method-call expression, emit inner call into synthetic `$__chain_tmp_N` local, register it temporarily in `arrayVars`, then emit outer method against it. |
...
Phases 42–46 close the remaining Go-by-Example test gaps and fix string escape sequences. Phases 47–49 round out the TypeScript language surface: class inheritance with virtual dispatch, the `Number` API, optional chaining, labeled `continue`, `switch` fallthrough, and chained array methods. Phase 50 completes the DLL model — `wasmtk bindgen` generates typed TypeScript host bindings from a `.wit` file so that wasic-compiled WASM modules can be loaded and called from a TypeScript host with full type safety and no manual `WebAssembly` API work. No phase requires an embedded runtime — everything maps to static WASM constructs. Features that need a runtime are listed under [Types Requiring `javyc`](#types-requiring-javyc) below.
