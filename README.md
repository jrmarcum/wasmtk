...
| 36 (2026-05-04) | Simple conditional types `T extends U ? X : Y` | **`expandConditionalTypes(src): string`:** new private method in `WasicTranspiler`, inserted after `expandGenerics` and before `expandNamespaces` in `transpile()`. Detects both generic (`type Name<T> = T extends U ? X : Y`) and non-generic (`type Name = A extends B ? X : Y`) conditional type declarations via a single regex; removes them from source; resolves all use sites at compile time. **Non-generic:** condition evaluated once; every bare occurrence of the type name replaced globally with the resolved concrete type. **Generic:** every `Name<ConcreteType>` use site is individually resolved by substituting the concrete type for the type parameter in the check and upper expressions, evaluating `extendsCheck`, and rewriting to the true or false branch. If the selected branch is the type parameter itself (passthrough pattern), the concrete type is substituted directly. **`extendsCheck(c, u)`:** same string → true; any numeric type (`i32`, `i64`, `f32`, `f64`, `number`) extends `number` → true; `bool`/`boolean` mutual extends → true; all else false. **Use positions:** variable annotations (`const x: Toggle<i32>`), function parameter types (`function f(val: NumCond<i32>)`), and function return types (`function area(): AsFloat<i32>`) — all resolved before `parseFunctions` runs. **`infer` not supported.** Four test files: `BasicConditionalType_36`, `ConditionalTypeParams_36`, `ConditionalTypeNonGeneric_36`, `Phase36Combined_36` — 124/124 suite |
