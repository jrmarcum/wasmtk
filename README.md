...
> in place, **the full `tests/wasm_wasi` suite is 306/306** (`core_`
...
> after a single-line `if` were silently dropped — regression `15_ElseChainForms`; Phase 52 — 2026-06-11 — added the leaf conveniences
...
> is now 306/306 under binaryen-ts 1.3.5.** The per-phase
...
**All 446/446 tests pass as of 2026-05-25** (270 wasic + Go-by-Example tests + 103 bindgen tests + 73 jstyper tests). Stage 0 Canonical ABI alignment (partial) complete — wasic exports `cabi_realloc` and uses an out-parameter string return convention (still a WASI-P1 core module + sidecar `.wit`; return side not yet canonical — see [`cmem/polyglot-producers.md`](cmem/polyglot-producers.md)).
...
| `string` (return) | `string` | allocate 8-byte return area via `cabi_realloc(0,0,4,8)`, pass as trailing arg, read `ptr`+`len` back via `DataView` → `TextDecoder` |
...
- *WASM → Host (return):* Canonical ABI out-parameter convention. The binding allocates an 8-byte return area via `_r = cabi_realloc(0, 0, 4, 8)` and passes `_r` as the trailing argument to the shim export. The shim writes `(ptr i32, len i32)` at offsets 0 and 4. The binding reads them back via `DataView.getInt32(_r, true)` / `DataView.getInt32(_r + 4, true)` and decodes via `TextDecoder`. wasic generates `$fn__cabi` shim wrappers automatically for all exported string-returning functions.
